...
**[ЗАРАЗ, DEV 2026-09-26]** Цілісний AI-only шлях розгорнуто. Генерація
(`LEXICON_AI_MODEL`) і незалежна перевірка/виправлення (`LEXICON_AI_REVIEW_MODEL`)
використовують `gpt-6-sol`, `reasoning_effort=medium`. Astra вимкнено на вимогу
власника; це також новий default backend-конфігурації. Порівняння Luna/Sol
виконується окремо від бази: тестова відповідь або схвалення моделі не змінює
живий словник. Старі `OPENAI_MODEL` та provider-таблиці нижче описують збережений,
але вимкнений у AI-only режимі ланцюг.
