...
Підтримувані мови (джерело істини — бекенд `supported-languages.ts:18`): `uk`, `en`, `de`, `es`, `fr`, `pl`, `it` (7 мов; у кожної є `speechTag` і `flag`).
...
2. Скрипт-евристика: кирилиця vs латиниця (`:34-44`) — швидко вирішує однолітерні слова й власні назви. Якщо тільки кирилиця і одна з мов кирилична → вона; аналогічно для латиниці.
3. `franc-min` для рядків довжиною ≥ 4 (`:46-52`), обмежений набором `FRANC_TO_ISO1` (`:9-18`: `ukr→uk, eng→en, rus→ru, pol→pl, deu→de, spa→es, fra→fr, ita→it`); результат приймається лише якщо він збігається зі `speakLang` або `learnLang`.
...
- Списки `isCyrillic`/`isLatin` (`:62-69`) ширші за реально підтримувані 7 мов (напр. `ru`, `pt`, `nl`, `sv`...). Це нешкідливо в межах детекції, але джерело істини підтримуваних мов — `SUPPORTED_LANGUAGE_CODES` (`supported-languages.ts:28`); `franc` обмежений 8 кодами в `FRANC_TO_ISO1` (включно з `ru`, якого немає в `SUPPORTED_LANGUAGES`).
