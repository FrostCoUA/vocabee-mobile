...
Документ описує мовну модель Vocabee: профільні мови, мову окремого словника, мовну пару Маркету, напрямок перекладу та його зв'язок зі STT/TTS, бекендне визначення мови (`lang-detect`) і візуальні теми словників (кольори + іконки).
...
### 1.4 Мовна пара Маркету `[НОВЕ D12]`

Top bar Маркету показує **обидві** ролі профільної пари окремими
клікабельними прапорцями:

- «Я знаю» → `userLanguage` / `knownLang`;
- «Я вивчаю» → `learningLanguage` / `learningLang`.

Робоче трактування D12: це не тимчасовий фільтр Маркету, а ще одна точка
редагування профільної пари. Тому вибір викликає наявні
`SelectSpeakingLanguage` / `SelectLearningLanguage`, персиститься й пушиться
на сервер. За D6 зміна не переписує мови існуючих словників; куплений
маркет-набір фіксує вибрану пару на момент покупки.

Кожен прапорець відкриває шторку своєї ролі:

1. перший рядок завжди **«Поміняти мови місцями»** — атомарний swap
   `userLanguage ↔ learningLanguage`;
2. далі — альтернативні підтримувані мови;
3. зі списку виключаються поточна мова цієї ролі й поточна мова іншої ролі,
   тому `knownLang != learningLang` зберігається без автокорекції після тапа.

Після вибору/свапу каталог повторно запитується для повної пари:
`learningLang` визначає мову контенту, а `knownLang` — мову перекладів.
Повна поведінка — [19](19-market-word-packs.md) §2–§3.
