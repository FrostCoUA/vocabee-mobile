...
2. **`currentUser()`** → `VocabeeEvent.ApplyAuthenticatedAccount(...)`: підтягує
...
|----------|-------------------|--------|
| Повторний запуск, немає мережі | `runStartupSync` кидає виняток на `currentUser`, він ковтається → застосунок працює з локальним станом (останній відомий профіль/словники з `PreferencesManager` + Room). | Без змін. Жодного блокувального екрана; синк повториться при наступному успішному старті/дії. |
...
- [ЗАРАЗ] Прострочений access (на старті чи під час відкритого застосунку) відновлює сам API-шар:
  bearer-виклик після першого `401` непомітно робить refresh під мьютексом і повторює свій запит;
  одночасні запити не змагаються за одноразовий refresh-token, а скасування викликача не губить
  щойно ротований токен (`NonCancellable`). Деталі — `docs/16` §16.5.
- Якщо **refresh остаточно не вдався**, `AuthTokenStore.markSessionNeedsReauth()` лише сигналізує UI:
  `MainApp` показує снекбар із пропозицією увійти через Google у профілі. Токени і Authenticated-стан
  **лишаються** — автоматичного sign-out немає; з акаунта виводить тільки кнопка «Вийти» (5.3).
