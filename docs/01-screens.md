...
| CreateDictionary | sheet | FAB / порожній стан Home; свайп «Змінити» (режим редагування) | `PrototypeBottomSheets.kt:134` |
| LearningLanguage («Я вивчаю») | sheet | Чип-прапорець у хедері Home/Практики | `PrototypeBottomSheets.kt` |
| DeleteDictionary | sheet | Свайп-видалення картки; «Видалити» в меню хедера деталей | `App.kt:999` |
| ClearDictionary («Очистити словник?») | sheet | «Очистити словник» у меню хедера деталей | `App.kt` (`ClearDictionaryConfirmationSheet`) |
...
### Візуальні стани
- **Порожній (`topics.isEmpty()`).** `EmptyHomeState`: ілюстрація, «Поки що порожньо», підказка, кнопка «Створити словник». FAB у цьому стані **не показується**, створення йде з центральної кнопки (CTA).
- **Непорожній.** Сітка 2-в-колонку (`LazyVerticalGrid`); картки `DictionaryCard` рендеряться в **реверсному** порядку (`topics.reversed()`) — найновіший зверху. Показується FAB.
- Немає окремих loading/error станів (дані беруться зі `store.state`).

### Групування за парами мов `[ЗАРАЗ]`
Сітка розбита на групи за парою **`sourceLanguage → targetLanguage`** словника (`groupTopicsByLanguagePair`, `VocabeeStore.kt`):
- **Робоча пара** (`learningLanguage → userLanguage` з профілю) завжди **перша і без заголовка** — це поточні словники.
- Решта пар — нижче, кожна під розділювачем `LanguagePairGroupLabel`: «ТЕМИ 🇩🇪 → 🇺🇦» (12.5sp/800 caps + лінія). Порядок «інших» груп — **за спаданням кількості словників**, за рівної кількості — стабільно за кодом мови-джерела, потім мови-перекладу.
- Порядок словників **усередині** групи не змінюється (Головна віддає найновіші першими).
- Зміна мови вивчення з чипа хедера **анімовано** переносить її групу вгору: `Modifier.animateItem(placementSpec = pairReorderSpec)` (spring `DampingRatioNoBouncy` / `StiffnessMediumLow`, ≈350мс) на картках і розділювачах; ключі — `topic.id` та `pair-<src>-<tgt>`.
- Метрики хедера («N словники · M слів зібрано») і гейти лімітів рахуються **по всіх** словниках, незалежно від груп.
...
### Хедер
`HomeHeader`: великий заголовок «Словники» + **чип-прапорець мови, яку вивчаю** (`LearningLanguageChip`) праворуч; метрики «N словники · M слів зібрано» (тільки коли `topicCount > 0`).
- `[ЗАРАЗ]` Замість логотипа стоїть чип: surface, бордер `Line` 1.5dp, r13, прапор 19sp + `ChevronDown` 13dp. Тап → шторка **LearningLanguage** («Я вивчаю»).
- Такий самий чип стоїть у хедері екрана «Тренування» (`PracticeSetupHeader`) — обидва відкривають ту саму шторку.
...
- **Свайп (дві дії)** `[ЗАРАЗ]`: жест вліво відкриває зону 76dp, поділену навпіл (`SwipeRevealDeleteContainer`, поріг 42% ширини або velocity 320):
  - зверху **«Видалити»** (фон `Red`, іконка `Close` 19dp + підпис 10sp/800) → `onTopicDeleteRequest` → шторка `DeleteDictionary`;
  - знизу **«Змінити»** (фон `Ink`, іконка `Edit` 18dp + підпис) → `onTopicEditRequest` → шторка `CreateDictionary(editTopicId = topic.id)` у режимі редагування.
  - Рядки слів у деталях використовують той самий контейнер **без** `onEditClick` — там лишається одна дія на всю висоту.
...
### Хедер (колапсуючий)
`DetailHeader` (`App.kt:2649`): кнопка «назад»; пілюля напрямку STT; **меню-кебаб**; назва + «N слів · оновлено …»; згортається по скролу (`detailHeaderCollapseFraction`, `App.kt:2086`), інтерполюючи висоту/розмір шрифту/позиції.

### Меню хедера (кебаб) `[ЗАРАЗ]`
`DetailHeaderMenuButton` — кнопка 40dp r13 (біле 18%) праворуч від пілюлі напрямку; три крапки малюються `KebabGlyph` (окремої іконки в `PrototypeIcon` немає). Тап → `Popup` (surface, r16, elev) із трьома діями:
- **«Змінити»** → `CreateDictionary(editTopicId)` — той самий шит, що й свайп-дія «Змінити» на картці Головної.
- **«Очистити словник»** → `ClearDictionary(topicId)` (див. шторку нижче).
- **«Видалити»** (текст `redText`) → `DeleteDictionary(topicId)` — той самий шит, що й свайп-видалення.

Меню в `Popup`, бо `Surface` хедера обрізає вміст своїми нижніми округленнями; закриття — тап поза меню / системний Back (`PopupProperties(focusable = true)`).
...
- **Єдиний режим.** `[ЗАРАЗ]` Перемикача «Класика / Слово в контексті» немає: вибрані словники завжди запускають спільну колоду флеш-карток. Ізольований context-mode код збережено, але від'єднано від UI.
- **Content.** Список `PracticeTopicPickerRow` (`App.kt:3673`) з чекбоксами; кожен рядок: іконка теми, назва, «N слів · X% знаю». Кнопка «Вибрати всі / Очистити» (`App.kt:3587`). Закріплений низ: «Вибрано: K тем · M слів» + кнопка «Почати тренування» (enabled коли `selectedWords > 0`). `[ЗАРАЗ]` Поверхня закріпленої панелі продовжується під верхні зовнішні округлення таб-бару, тому в кутках не просвічує фон екрана.
- **Групи пар мов** `[ЗАРАЗ]`. Хедер має той самий **чип-прапорець** мови вивчення, що й Головна (відкриває шторку «Я вивчаю»); рядки згруповані тим самим `groupTopicsByLanguagePair` — робоча пара вгорі без заголовка, решта під розділювачами «ТЕМИ 🇩🇪 → 🇺🇦». Деталі — `docs/11-practice-training.md`.
- **Дії.** Тап по рядку → toggle теми **в межах однієї пари** (вибір рядка з іншої групи скидає попередній вибір); «Вибрати всі / Очистити» діє лише на активну пару; «Почати» → `practiceStarted = true` (якщо вибір не порожній).

### Фаза 2 — колода карток
- **Fullscreen-сесія.** `[ЗАРАЗ]` Після старту нижній таб-бар приховується, а колода й done-стан займають увесь екран. Праворуч від заголовка «Тренування» є хрестик. Хрестик і системний Back у незавершеному раунді відкривають bottom sheet «Перервати тренування?» з діями «Перервати» / «Продовжити»; уже записані відповіді не відкочуються. У done-стані хрестик одразу повертає до вибору тем.
- **Формування колоди (`buildPracticeDeckKeys`).** Зі слів обраних тем; стабільне сортування за ключем перед shuffle із сидом `shuffleSeed`, потім сортування за зростанням `knowledgePercent` (спершу гірше вивчені), `take(10)` — максимум 10 карток на раунд. Стан сесії ключується множинами ID тем/карток, тому зміна порядку даних після sync не перетасовує колоду й не скидає прогрес.
- **`PracticeFlipCard` у `HorizontalPager`.** `[ЗАРАЗ]` Кожна pager-сторінка має власний flip/answer-стан, тому нове слово завжди стартує лицем, незалежно від стану попередньої картки. Ручний user-scroll pager вимкнений (горизонтальний жест лишається за flip); «Знаю»/«Далі» анімовано слайдять до наступної сторінки за ~420 мс. Збільшена картка заповнює весь доступний простір між прогресом і відповідями. Flip-анімація по осі Y (`rotationY`, `tween(460)`); видима грань визначається фактичним нормалізованим кутом, тому під час зворотного flip переклад лишається на своїй стороні до проходження ребра 90°. Тап обертає картку стандартно на `+180°`, горизонтальний свайп/флінг ліворуч — на `−180°`, праворуч — на `+180°`. Лице: чип теми, чипи типу/регістру (`Фраза`, `Вислів`, `Абревіатура`, `Сленг`…), source, IPA, рівень засвоєння, TTS і sense-specific або lexical usage-example з підсвіченою цільовою одиницею. Зворот: головний переклад, а для нетипових одиниць також «Що означає», розшифровка обома мовами, дослівний переклад і внизу переклад usage-example. **Перше ручне відкриття перекладу автоматично фіксує «Не знаю» (`−20%`) і замінює кнопки на «Далі»;** наступні flip-жести вже не змінюють оцінку. Тап по динаміку лише озвучує source-слово.
- **Кнопки відповіді / стани кнопок.**
  - До відповіді: дві кнопки — «Не знаю» (`answerUnknown` → `-20%`, перевертає картку, чекає «Далі») і «Знаю» (`answerKnown` → `+20%`, наступна).
  - Після «Не знаю» **або ручного відкриття перекладу**: одна кнопка «Далі» (`moveNext`), стрілка стоїть праворуч від напису.
...
### Запрошення й підтримка
- `[ЗАРАЗ]` «Запросити друзів» відкриває push-екран `InviteFriendsScreen`: персональна реферальна лінка й QR від `/v1/referral/me`, копіювання лінка та системний Share. Android Share одночасно передає текст (`EXTRA_TEXT`) і PNG QR `1024×1024` (`EXTRA_STREAM` через `FileProvider`). Сума в контракті/UI — **+50 монеток кожному**.
- `[ЗАРАЗ]` «Допомога та підтримка» відкриває `HelpSupportScreen` із формою звернення до `/v1/support`.
...
## CreateDictionary
`CreateDictionarySheet` — `PrototypeBottomSheets.kt`. Відкривається з FAB/empty Home (через гілку `onCreateClick`, `App.kt:790`).
- **Елементи `[ЗАРАЗ]`.** Пара прапорів «🇬🇧 → 🇺🇦» у **хедері** шита (слот `titleAccessory`, `SheetLanguagePairChip`) — окремої мовної стрічки в тілі більше немає; поле «Назва теми» (autofocus, ліміт 28 символів, `SheetTextField`); пікер іконок `IconPicker` 5×4 (20 іконок, D7); палітра кольорів `SwatchPalette` 2×6 (12 акцентів); кнопка «Створити» (`SheetPrimaryButton`, 56dp r16, disabled поки назва порожня).
- **Порядок іконок `[ЗАРАЗ]`.** Рендер іде в **дисплейному** порядку борду (`PrototypeTopicIconsPickerOrder`), а зберігається **канонічний** індекс (`prototypeTopicIconStorageIndex`) — тому зміна порядку показу не перефарбовує іконки наявних словників.
- **Ліміт.** Якщо `existingDictionariesCount >= 5` → `LimitNote` «Ти створив(ла) максимум 5 словників…». `[ЗАРАЗ]` Це косметична нота з макета; реальний гейт — `FREE_DICTIONARY_LIMIT = 2` + 10 монеток за наступний словник, і виконує його `onCreateClick` ще до відкриття шторки. Текст ноти суперечить чинній економіці (D1) — кандидат на переписування.
- **Дія.** «Створити» → `CreateTopic(title, coverIndex, iconIndex)` + `syncVocabularyNow()` + закриття + **авто-перехід** у новий `TopicDetail` (`App.kt:795–802`).
- `[ЗАРАЗ]` **Режим редагування.** `PrototypeSheet.CreateDictionary(editTopicId)` — відкривається свайп-дією «Змінити» на картці Головної **або** дією «Змінити» в меню хедера деталей. Поля префілляться наявним словником, заголовок → «Редагування», CTA → «Зберегти», нота про ліміт не показується. Дія → `UpdateTopicAppearance(topicId, title, coverIndex, iconIndex)` + sync + закриття (**без** переходу в деталі). Пара мов у цьому режимі не редагується (D6), монетки не списуються (платне лише створення понад ліміт).
- `[НОВЕ]` (D6) Дозволити **оверрайд пари мов** для цього конкретного словника (зараз чип у хедері лише read-only).
...
## ClearDictionary — «Очистити словник?» `[ЗАРАЗ]`
`ClearDictionaryConfirmationSheet` — `App.kt`. Відкривається дією «Очистити словник» у меню-кебабі хедера деталей (`PrototypeSheet.ClearDictionary(topicId)`).
- **Призначення.** Прибрати **всі слова** словника разом із прогресом засвоєння; **сам словник лишається** (на відміну від `DeleteDictionary`).
- **Елементи.** «Усі **N слів** у словнику «X» буде видалено разом із прогресом засвоєння. Сам словник залишиться.» (число + одиниця виділені `Ink`/bold через `buildAnnotatedString`); червоне «Цю дію не можна скасувати.»; лейбл «Введи ОЧИСТИТИ, щоб підтвердити» + поле `SheetTextField`; кнопки «Скасувати» (neutral) / «Очистити» (danger, 56dp r16).
- **Гейт кнопки.** «Очистити» **disabled** (`Red` з alpha .45), поки введене ≠ контрольній фразі: `isClearDictionaryConfirmed(input)` → `input.trim() == ClearDictionaryConfirmationPhrase` (**«ОЧИСТИТИ»**, **регістр важливий**, пробіли по краях прощаємо). Тест — `VocabeeStoreTest.clearButtonUnlocksOnlyForTheExactControlPhrase`.
- **Дія.** `ClearTopicWords(topicId)` → `syncVocabularyNow()` → закриття + снекбар «Словник очищено — видалено N слів». **Undo немає** (на відміну від видалення слова) — саме тому шторка вимагає контрольної фрази. Монетки не повертаються (D3). Деталі поведінки та шляхи в БД — [07](07-deletion.md).

## LearningLanguage — «Я вивчаю» `[ЗАРАЗ]`
`LearningLanguageSheet` — `PrototypeBottomSheets.kt`. Відкривається чипом-прапорцем у хедері Головної та екрана «Тренування» (`PrototypeSheet.LearningLanguage`).
- **Елементи.** Заголовок «Я вивчаю» + пара прапорів «мова-вивчення → рідна» у хедері шита (слот `titleAccessory`); список усіх `PrototypeLanguages`, крім рідної.
- **Активні** рядки — мови, що вже мають словники **в парі з рідною** (плюс поточна вибрана, навіть без словників): прапор + назва + лічильник «N словники» + selected-стан (tint + purple border + чек-бейдж 22dp).
- **Неактивні** — мови без словників у парі: `alpha .42`, `clickable(enabled = false)`, без лічильника.
- Порядок: спершу активні (за спаданням кількості словників, далі за назвою), потім неактивні за назвою.
- **Футер-примітка.** «Неактивні — підтримувані мови без словників у парі з 🇺🇦. Новий словник іншої мови створюється через профіль або кнопку «+».»
- **Дія.** `onPick(code)` → `SelectLearningLanguage` + `pushProfileSettings()` + закриття. Група обраної пари анімовано переїжджає вгору на Головній і в сетапі тренування.
...
- **Елементи `[ЗАРАЗ]`.** Пояснення, чому не можна авто-мержити; **дві картки порівняння** зі стрілкою між ними — «На телефоні» (іконка `Book`) і «На бекенді» (іконка `Globe`), кожна з реальними числами «N словників · M слів» (локальні — `localAnonymousTopicCount()`/`localAnonymousWordCount()`, серверні — з `SyncResponse`); три дії з підписом наслідку під кожною: «Залити локальний стан / серверні дані буде перезаписано» (primary), «Взяти стан з бекенда / локальні N слів буде видалено» (neutral), «Увійти іншим email / нічого не зміниться» (ghost).
- **Дії (`App.kt:894–922`).**
