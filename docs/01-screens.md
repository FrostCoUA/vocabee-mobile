...
Повна карта UI-шару клієнта (Kotlin Multiplatform / Compose). Документ описує **усі екрани**, **усі bottom sheets (шторки)**, оверлей додавання слова та навігацію. Для кожної поведінки явно вказано `[ЗАРАЗ]` (як працює в поточному коді) та `[НОВЕ]` (затверджена зміна за рішеннями D1–D12).
...
- «Доданість» рахується наживо по **парі (слово, переклад)** з поточного стану словника: `option.isSavedIn(savedWordKeys)` (`AddWordOverlay.kt`) — тап `+`/`✓` перемикає рядок в обидва боки без нового пошуку. Збережений `run→серія` не позначає ✓ на `series→серія`. Знімок `alreadyAdded` з пошуку на екран не впливає взагалі — підпису «додано раніше» немає (рендер `TranslationOptionNote` прибрано історично).
