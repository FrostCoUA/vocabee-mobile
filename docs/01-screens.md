...
- **`PracticeFlipCard`.** `[ЗАРАЗ]` Збільшена картка заповнює весь доступний простір між прогресом і відповідями. Flip-анімація по осі Y (`rotationY`, `tween(460)`). Лице: чип теми, source-слово, IPA, рівень засвоєння `PracticeKnowledgeLevel` (5 поділок), окрема кнопка озвучення та контекстне речення саме цього `WordEntry` (якщо збережене). Зворот: переклад. Тап по поверхні картки лише перемикає грань; тап по динаміку лише озвучує source-слово й не зараховує відповідь. Коротке блокування flip одразу після старту не дає стартовому тапу «протекти» в нову картку.
