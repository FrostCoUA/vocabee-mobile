...
- **`PracticeFlipCard`.** `[ЗАРАЗ]` Збільшена картка заповнює весь доступний простір між прогресом і відповідями. Flip-анімація по осі Y (`rotationY`, `tween(460)`); видима грань визначається фактичним нормалізованим кутом, тому під час зворотного flip переклад лишається на своїй стороні до проходження ребра 90°. Тап обертає картку стандартно на `+180°`, горизонтальний свайп/флінг ліворуч — на `−180°`, праворуч — на `+180°`. Лице: чип теми, source-слово, IPA, рівень засвоєння `PracticeKnowledgeLevel` (5 поділок), окрема кнопка озвучення та контекстне речення саме цього `WordEntry` (якщо збережене); цільове слово/словоформа в реченні підсвічується жовтим тим самим `highlightedSentence`, що й у контекстному тесті. Зворот: переклад. **Перше ручне відкриття перекладу автоматично фіксує «Не знаю» (`−20%`) і замінює кнопки на «Далі»;** наступні flip-жести вже не змінюють оцінку. Тап по динаміку лише озвучує source-слово. Коротке блокування flip одразу після старту не дає стартовому тапу «протекти» в нову картку.
