...
- «Доданість» рахується наживо по **парі (слово, переклад)** з поточного стану словника: `option.isSavedIn(savedWordKeys)` (`AddWordOverlay.kt`) — тап `+`/`✓` перемикає рядок в обидва боки без нового пошуку. Збережений `run→серія` не позначає ✓ на `series→серія`. Знімок `alreadyAdded` з пошуку впливає лише на підпис «додано раніше».
