...
**[ЗАРАЗ→змінено фазами 2–4]** Одиночний `WordRow` (картка на РЯДОК) **видалений** —
екран деталей рендерить лише `WordGroupRow`, і його одиниця тепер **сенс-група**
(D16, [01](01-screens.md) §6), а не source-слово.

- `hasDetails = displayDetails != null && !displayDetails.isEmpty`.
- Картка розгортувана, якщо `canExpand = hasDetails || nearbyTranslations.isNotEmpty() || sourceTextOverflows || translationTextOverflows || nearbyTextOverflows`.
- Тап (`clickable enabled = canExpand`) тогглить `expanded`. У розгорнутому стані source/переклад/рядок «близьких» показуються повністю (`maxLines = Int.MAX_VALUE`).
- Шеврон `ChevronDown` обертається на 180° при `expanded && canExpand`, фарбується `accent`, інакше `Muted3`.
- При `expanded && displayDetails != null` рендериться `WordDetailsBlock`.

#### Звідки береться блоб картки — `WordGroup.displayDetails` `[НОВЕ, фаза 3]`

Заголовок картки показує пару **представника** (найновішого запису групи за
`addedAtEpochMillis`), тож і деталі мають бути його:

1. `representative.details`, якщо він непорожній — звичайний випадок;
2. інакше — перший непорожній блоб у члена **того самого сенсу**. Перетин
   `senseMergeKeys()` тут **обовʼязковий**: без нього запис-місток `[k1, k2]`
   протягнув би під заголовок `k1` блоб сусіда `[k2]` з чужими сенсами;
...
> мітка може бути **однакова** — тоді `maxByOrNull` віддає **перший** запис у списку,
> тобто поведінка вироджується у стару «перший у групі». Це свідомо прийнято: різниця
...
>
> **[ЗАРАЗ→змінено фазами 2–4] Вимикача скоупу більше немає.** `WordDetailsBlock` мав прапорець `scopeToAttributedSense` (і `displayContent(scope)` — параметр), яким групова картка вимикала звуження й показувала всі сенси блоба. Усі продакшн-кол-сайти передавали `true`, а відколи картка = один сенс, «показати всі сенси атрибутованого блоба» не потрібне нікому — прапорець видалено, `displayContent()` завжди скоупить, а автоматичний фолбек «немає атрибуції → усі сенси» лишився всередині.
...
**[ЗАРАЗ]** IPA показується **у згорнутому й розгорнутому стані** поряд із source-словом, якщо він не порожній: `Muted2`, 13sp, SemiBold. Джерело — `WordEntry.ipa` (снапшот, §5), не з `details`. **[ЗАРАЗ→змінено фазами 2–4]** Картка бере `WordGroup.ipa` — перший непорожній серед записів групи (а не `word.ipa` одного рядка), тож представник без транскрипції не лишає картку без неї.
