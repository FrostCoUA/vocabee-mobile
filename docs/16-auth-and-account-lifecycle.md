...
- Єдине джерело правди для bearer-сесії. Access/refresh і монотонне runtime-покоління
  тримаються разом; `token: StateFlow<String?>` лишається реактивним потоком access.
- `captureSession()` атомарно фіксує access, refresh і generation для одного запиту.
  Запит і його retry використовують тільки цю bearer-сесію, а не перечитують
  довільний новий токен з prefs після account switch.
- `replaceSession(AuthTokensResponse)` починає нове покоління при Google-вході;
  `rotateSession(..., expectedGeneration)` записує одноразову refresh-ротацію лише
  якщо logout/новий login не змінили власника; `clear()` також збільшує покоління.
- Токени ініціалізуються з prefs при створенні store, а далі змінюються через ці
  серіалізовані операції; `current()`/`refreshToken()` читають поточний in-memory стан.
...
- Refresh зберігається в `PreferencesManager` (`accessToken`, `refreshToken`;
  також `currentUserId`, `lastAuthenticatedUserId` і per-user
  `lastSyncAt(userKey)`/`localRevisionEpochMillis(userKey)`).

### Виклики API (KtorVocabeeApi)

**[ЗАРАЗ]**: `loginWithGoogle` → `POST /v1/auth/google`, у відповідь
`tokenStore.replaceSession(tokens)`. Ротацію (`POST /v1/auth/refresh`) робить **тільки**
приватний `renewSession` всередині `KtorVocabeeApi` — публічного `refreshSession` у
`VocabeeApi` немає. Усі bearer-виклики фіксують `AuthSessionLease`; після `401` запит
може повторитися лише в тому самому generation. Успішна відповідь і anonymous
fallback також проходять post-check: відповідь A не повертається UI після переходу в
B/logout. Захисти в `renewSession`:

- `Mutex` серіалізує одночасні refresh-запити (rotation одноразова — паралельні 401 не можуть відкликати сесію одне одного).
- Сам обмін загорнутий у `NonCancellable`: якщо корутину викликача скасували, вже
  розпочатий exchange завершується. Але `rotateSession` комітить результат лише для
  того самого generation; stale refresh акаунта A після logout або входу в B не може
  відновити/перезаписати токени.
...
3. Після успіху під per-user `VocabularySyncCoordinator` виконуємо PUSH, якщо auth
   namespace уже має dirty revision/`Pending*`-рядки, інакше тягнемо full snapshot;
   account switch або локальна правка під час запиту не можуть застосувати відповідь.
   Далі вирішуємо розвилку:
   ...
   - **Локальних нема** → `applyServerSnapshot(user.id, serverSnapshot)` під успішним lease → «Акаунт синхронізовано».
   У конфлікті «Взяти серверне» не застосовує кешований snapshot напряму: повторно
   перевіряє captured revision/account, робить fresh full pull і лише тоді замінює Room.
...
3. Під `VocabularySyncCoordinator` фіксуються user id і його локальна revision.
   Якщо `localRevisionEpochMillis(user.id) > 0` або Room має `Pending*`-рядки —
   виконується PUSH. Інакше — delta від `lastSyncAt(user.id)`; відставання
   `appliedLexiconSchemaVersion(user.id)` форсить один full pull (D15).
4. Перед повною заміною Room coordinator повторно перевіряє акаунт і revision.
   Account switch, нова локальна правка або паралельний sync не можуть застосувати
   stale відповідь чи очистити dirty-стан іншого користувача.
5. Будь-яка мережева помилка ковтається (`catch (_) {}`): лишаємось офлайн/локально,
   явний вхід покаже помилки.
...
**[ЗАРАЗ]** Обидві точки в `App.kt` — кнопка профілю та гілка sync-конфлікту
«інший email» — викликають suspend `clearAuthenticatedSessionForAnotherEmail()`.
Вона через `LocalSessionController` робить `AuthTokenStore.clear()`, тобто атомарно
чистить токени й інвалідовує generation усіх старих запитів, а потім чистить
`currentUserId`, скидає conflict/sheet і виконує `signOutKeepLastUserState()`.
