...
**[ЗАРАЗ]** Глобальний префікс об'єднаного gateway — `v1` (`vocabee-gateway/src/main.ts:13`). Нижче поточні шляхи наведено повністю (`/v1/...`); цільові v2-контракти позначено окремо.

---

## 1. Ендпоінти gateway

### 1.0 [НОВЕ] Межі сервісів і rollout-контракт

| Компонент | Володіє | Не володіє |
|---|---|---|
| `client-gateway` | mobile API, users/auth, wallet/economy policies, rewarded ads, premium, topics, saved words, sync, lookup receipts, feedback outbox | lexicon content, provider generation/cache, upstream provider API keys |
| `dictionary-gateway` | languages, lexicon, translation generation/cache, provenance/revisions, imports, quality feedback, consumer API keys, upstream provider configuration | app JWT/users, premium, beecoins, topics і saved words |
| `client-admin-web` | адміністрування users/wallet/premium/economy policies/admin accounts через `client-gateway` | прямий DB-доступ |
| `dictionary-admin-web` | lexicon/provenance/imports/feedback/provider credentials/API consumers через `dictionary-gateway` | прямий DB-доступ |

Мобільний застосунок викликає **лише `client-gateway`**. `client-gateway` звертається
до `dictionary-gateway` з unlimited App Service Key; ключ ніколи не потрапляє у мобільний
клієнт. Dictionary API app-neutral: без user id, app JWT, email, premium або балансу.

```http
GET  client-gateway /v1/search                         # compatibility facade
GET  dictionary-gateway /v1/search                    # X-API-Key, app-neutral
POST client-gateway /v2/translation-lookups           # free lookup receipt
POST client-gateway /v2/topics/{topicId}/words/from-result
POST client-gateway /v2/translation-feedback
```

Legacy v1 pricing лишається **без змін лише на час rollout**. D11 (`searchCost=0`,
charge за saved result) не можна enforce до релізу сумісного v2-клієнта. Обидва
gateway мають власні Swagger UI `/docs`, OpenAPI JSON `/openapi.json` і
`/v1/health`; client Swagger описує bearer auth, dictionary Swagger — `X-API-Key`.
...
### 1.5 [ЗАРАЗ] Search / Lexicon (`/v1/search`)
...
`VariantDto`: `knownWord`, `learningWord`, `ipa?`, `audioUrl?`, `partOfSpeech[]`, `examples[]` (`{text, translation?}`), `senses[]` (`{definition, partOfSpeech?, tags[], examples[], synonyms[], antonyms[]}`), `synonyms[]`, `antonyms[]`, `forms[]` (`{text, tags[]}`), `senseIndex?` (індекс sense'а, що його рендерить цей переклад; null — не атрибутовано), `source`, `origin`, `confidence?`, `isPrimary`, `cached`, `match` (`exact`\|`prefix`).
...
### 3.2 [ЗАРАЗ] Postgres об'єднаного gateway — `vocabee-gateway/src/db/schema`

> **[НОВЕ, split]** Цільовий стан має дві логічні БД/credentials:
...
> тримає snapshot і opaque dictionary identifiers. На першому етапі обидва процеси
> можуть працювати з поточною схемою, але не додають нових cross-domain залежностей.
