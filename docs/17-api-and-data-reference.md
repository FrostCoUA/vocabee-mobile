...
**[ЗАРАЗ, 2026-09-26]** У списку перекладів кожна AI-пара має пояснення свого
значення мовою перекладу зі звіреного `aiTargetSnapshot`. Однаково написані пари
з різними sense ID залишаються окремими записами з власними діями; пояснення
дає змогу відрізнити їх без відкриття деталей (наприклад, `drill → дрил` як
мавпа або музичний жанр).
Шрифти обох production-збірок зберігаються окремими файлами, щоб відповідати
чинному `font-src 'self'` без послаблення CSP.

**[ЗАРАЗ, 2026-09-26] JSON-експорт картки перекладу.** Біля заголовка деталей
доступна дія «Завантажити JSON». Вона завантажує UTF-8 `.json` із
`formatVersion: 1`, `exportType: "vocabee-translation-detail"`, `exportedAt` та
`translation` — поточними даними admin GET: вихідне/перекладене слово,
значення, приклади, форми, зв'язки, альтернативи та безпечні метадані.
Файл формується локально з уже завантаженої картки, не викликає AI і не змінює
базу. Доступний також адміністратору з правом лише читання. Це експорт
поточного admin DTO, а не оригінальна відповідь моделі чи повний AI-cache entry;
він зберігає обмеження й фільтрацію endpoint деталей. Ім'я файлу містить слово,
мовний напрямок і ID перекладу.

**[ЗАРАЗ, 2026-09-26] Структура сторінки перекладу.** Заголовок показує
пару слів, напрямок перекладу, вимову, частину мови та стилістичні позначки.
Сторінку поділено на три розділи:
- «Цей переклад» (відкритий спочатку): пояснення вибраного значення,
  двомовні приклади, синоніми/антоніми та згорнуті форми обох слів.
- «Інші значення»: картки альтернативних перекладів із власними поясненнями
  й посиланнями; повні статті вихідного та перекладеного слова розгортаються
  окремо, як і кожне значення всередині.
- «Дані запису»: походження, ID, дати, безпечні метадані, оцінка якості,
  позначення неякісного перекладу та видалення/відновлення згідно з правами.

Вибране AI-значення визначається через збережені `aiSourceSnapshot.details.senseKeys`
і справжні sense ID, цільове значення — через translation ID. Вимова й форми
беруться тільки зі snapshot відповідного значення; чужі приклади та зв'язки
із загальної статті не потрапляють до першого розділу. Якщо точної прив'язки
немає, сторінка пояснює це і залишає повні дані в «Інших значеннях» без
припущень за текстом або порядком. Оцінка прикладів зберігає їхні справжні ID.
JSON-експорт і далі містить повний завантажений DTO незалежно від вибраного
розділу. При переході до іншого перекладу відкривається його перший розділ.
Вкладки підтримують клавіатуру, текст переноситься, компонування адаптується
до вузького екрана.
