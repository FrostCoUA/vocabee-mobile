...
`client-admin-web` ще **[МАЙБУТНЄ]**; API і Swagger уже реалізовані. Mobile app ці
routes не викликає.
...
| `0013_dictionary_api_consumers.sql` | Плани й consumers Dictionary API, digest-only ключі, consumer-level quota counters, append-only usage/audit та protected seeds `system-unlimited`, `external-standard`, `client-gateway`. |
