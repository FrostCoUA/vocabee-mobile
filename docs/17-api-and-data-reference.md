...
| `dictionary-gateway` | app-neutral search, lexicon, translation generation/cache, DB-backed consumer keys/quotas, consumer/key lifecycle, dashboard/current-lexicon/provider-status/usage/audit admin API | immutable provenance/revisions, reviewed imports, quality feedback, provider-secret write API, optional MCP |
...
| `dictionary-admin-web` | Task 1 read UI реалізований та інтегрується: dashboard/current translations/provider status/consumers+keys/usage/audit через `dictionary-gateway` | **[НОВЕ]** key mutation + one-time raw dialog і container deployment; **[МАЙБУТНЄ]** provider-secret/import/revision/feedback/MCP UI |
...
POST client-gateway /v2/translation-feedback          # [НОВЕ]
```
...
`dictionary:lexicon:read`, `dictionary:consumers:read`,
...
| GET | `/v1/admin/lexicon/translations` | `dictionary:lexicon:read` | Cursor-list current state; `sourceLang?`, `targetLang?`, `source?`, `origin?`, `providerTier?`, `q?` |
| GET | `/v1/admin/lexicon/translations/:translationId` | `dictionary:lexicon:read` | Current-state-only detail; це ще не immutable revision history |
...
`dictionary-admin-web` Task 1 read UI **[ЗАРАЗ]** реалізований і проходить integration:
dashboard, current translations, provider status, consumers/keys, usage та audit.
Key create/rotate/revoke controls і one-time raw-key dialog, а також окремий container/
Compose deployment ще **[НОВЕ]**. Browser не отримує consumer `X-API-Key`.
...
**[МАЙБУТНЄ, після cutover]** Encrypted provider-secret writes, immutable translation
revisions/provenance, reviewed import API, quality feedback/export і optional MCP
...
sense'а, що його рендерить цей переклад; null — не атрибутовано), `source`, `origin`,
...
### 1.10 [НОВЕ] Feedback про поганий переклад

| Метод | Сервіс і шлях | Суб'єкт | Призначення |
|---|---|---|---|
| POST | `client-gateway /v2/translation-feedback` | JWT | Прийняти `resultId`, reason і optional comment; поставити dictionary feedback в outbox |

Reasons: `incorrect`, `wrong_language`, `unnatural`, `wrong_sense`, `bad_example`,
`other`. `client-gateway` резолвить exact `translationId`/`revisionId` зі свого receipt,
передає HMAC-псевдонімний reporter ref без app PII, а `dictionary-gateway` зберігає
feedback ідемпотентно. Feedback сам по собі не повертає монетки.
...
> **[НОВЕ] Поля знань D10** — у поточному `WordEntry` НЕ існують. Гібрид пріоритет+Leitner потребує: `timesCorrect`, `timesWrong`, `boxLevel`, `lastReviewedAt`, `dueAt`. Поточний бекенд/клієнт мають лише `knowledgePercent`. Канон полів — `11-practice-training.md`; план персистенції — §3 (Room) і §3 (Postgres, майбутня `0014_training_fields.sql` **[НОВЕ, ще не створена]**).

### 2.4 WordDetails / WordSense / WordForm (`VocabularyModels.kt:8-40`)

- **`WordDetails`**: `senseIndex: Int?` (значення пари з бекендової атрибуції), `senses: List<WordSense>`, `synonyms: List<String>`, `antonyms: List<String>`, `forms: List<WordForm>`, `partOfSpeech: List<String>`; обчислюване `isEmpty`. Read-only на клієнті, серіалізується в Room як один JSON-блоб.
...
> **[НОВЕ] Поля тренування D10** для `topic_words` (майбутня `0014_training_fields.sql`, ще не створена): `times_correct` int def 0, `times_wrong` int def 0, `box_level` int def 0 (Leitner), `last_reviewed_at` timestamptz, `due_at` timestamptz.
...
| `0014_training_fields.sql` **[НОВЕ, ще не створена]** | Майбутня наступна міграція після наявної `0013`; `topic_words`: `times_correct`, `times_wrong`, `box_level` (def 0), `last_reviewed_at`, `due_at` (**D10**). |
...
4. Поля знань D10 (`timesCorrect/timesWrong/boxLevel/lastReviewedAt/dueAt`) відсутні і в клієнті, і в Postgres/Room — є лише `knowledgePercent`. **[НОВЕ]** (майбутня `0014_training_fields.sql`, ще не створена, + bump Room до v5).
