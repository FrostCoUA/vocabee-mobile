...
|---|---|---|
| `client-gateway` | mobile API, users/auth, wallet, rewarded ads, premium, topics, saved words, sync і compatibility search facade | lookup receipts, feedback outbox, D11 save-time economy |
| `dictionary-gateway` | app-neutral search, lexicon, translation generation/cache, provider configuration через env | provenance/revisions, imports, quality feedback, DB-backed consumer keys/quotas, provider-secret admin API |
| `client-admin-web` | — | **[МАЙБУТНЄ]** users/wallet/premium/economy/admin accounts через `client-gateway`, без прямого DB-доступу |
| `dictionary-admin-web` | — | **[МАЙБУТНЄ]** lexicon/provenance/imports/feedback/provider credentials/API consumers через `dictionary-gateway`, без прямого DB-доступу |
...
`dictionary-gateway` по внутрішньому HTTP з unlimited App Service Key; ключ ніколи не
потрапляє у мобільний клієнт. Dictionary API app-neutral: без user id, app JWT, email,
premium або балансу. Поточний bootstrap підтримує один env-ключ; DB-backed видача,
ротація й quota для зовнішніх consumers лишаються **[НОВЕ]**.
...
> **[НОВЕ] Поля знань D10** — у поточному `WordEntry` НЕ існують. Гібрид пріоритет+Leitner потребує: `timesCorrect`, `timesWrong`, `boxLevel`, `lastReviewedAt`, `dueAt`. Поточний бекенд/клієнт мають лише `knowledgePercent`. Канон полів — `11-practice-training.md`; план персистенції — §3 (Room) і §3 (Postgres, майбутня `0013_training_fields.sql` **[НОВЕ, ще не створена]**).
...
> **[НОВЕ] Поля тренування D10** для `topic_words` (майбутня `0013_training_fields.sql`, ще не створена): `times_correct` int def 0, `times_wrong` int def 0, `box_level` int def 0 (Leitner), `last_reviewed_at` timestamptz, `due_at` timestamptz.
...
| `0012_client_admin.sql` | `users.account_status` + moderation audit fields/index; `admin_accounts`, append-only `admin_audit_events` і `rewarded_ad_events`. |
| `0013_training_fields.sql` **[НОВЕ, ще не створена]** | Майбутня наступна міграція після наявної `0012`; `topic_words`: `times_correct`, `times_wrong`, `box_level` (def 0), `last_reviewed_at`, `due_at` (**D10**). |
...
4. Поля знань D10 (`timesCorrect/timesWrong/boxLevel/lastReviewedAt/dueAt`) відсутні і в клієнті, і в Postgres/Room — є лише `knowledgePercent`. **[НОВЕ]** (майбутня `0013_training_fields.sql`, ще не створена, + bump Room до v5).
