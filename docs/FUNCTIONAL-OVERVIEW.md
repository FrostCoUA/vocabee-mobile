...
>
> Позначки стану в усьому файлі: **[ЗАРАЗ]** — поведінка в поточному коді; **[ЗАРАЗ→змінено фазами 2–4]** — так було до сенс-груп (D16), поруч описано, як стало; **[НОВЕ]** — затверджена зміна (журнал рішень D1–D16); **[МАЙБУТНЄ]** — заплановано пізніше / TBD.
>
> Джерело істини рішень — журнал **D1–D16** у [00-overview-and-decisions.md](00-overview-and-decisions.md). Деталі кожного домену живуть у профільних документах, а технічні API/дані — у [17-api-and-data-reference.md](17-api-and-data-reference.md) та відповідному профільному розділі.
...
| ~~Морф-оверлей «Додати слово»~~ | Композабл `AddWordOverlay` **видалено** (мертвий код): додавання слова живе в доку деталей. Опис морфа лишається як історія дизайну. | [ЗАРАЗ→змінено фазами 2–4] | [01](01-screens.md) §7, [13](13-add-word-and-ai-search.md) §1 |
...
| Один айтем на сенс у пошуку | Варіанти одного значення зливаються: головний переклад + перший рядок сенсу, у розгорнутому стані — «Близькі за значенням» зі своїм `+`/`✓` на кожен синонім. | [НОВЕ] фаза 2 | [13](13-add-word-and-ai-search.md) §5.1 |
| Скарга на переклад — кнопка-дія | Рядок результату має кнопку «Поскаржитись на переклад» (прапорець, тап-таргет ≥40dp) замість тексту-вироку «Неякісний переклад»; надсилається opaque `translationId` саме цієї опції, без видалення рядка, один раз на target (`+1`). | [ЗАРАЗ→змінено фазами 2–4] | [13](13-add-word-and-ai-search.md) §5.2, [17](17-api-and-data-reference.md) §1.10 |
| Виявлення дубліката (позначка `+`/`✓`) | Живий локальний `Set` ключів пари (слово, переклад) → двобічний toggle `+`↔`✓` (`alreadyAdded` обчислюється, але споживача в UI не має). | [ЗАРАЗ] | [13](13-add-word-and-ai-search.md) §6 |
| Дедуп збереження — по сенс-групі | Гейт `addWord` рахує **перетин `senseKeys`** з усіма записами словника, а не пару: той самий сенс двічі не зберігається, різні сенси одного слова — легальні окремі записи. Кандидат без атрибуції гейт не чіпає. | [НОВЕ] фаза 2 · D16 | [13](13-add-word-and-ai-search.md) §6.2, [03](03-data-caching.md) §6.5 |
| Снекбар відмови збереження | «Цей сенс уже у словнику» — видимий снекбар **над IME** усередині екрана деталей (маршрутний хост — фолбек); монетка не списується, рядок не створюється. | [НОВЕ] фаза 2 | [13](13-add-word-and-ai-search.md) §6.2 |
| Групування слів — картка на СЕНС | Записи одного source із **перетином `senseKeys`** схлопуються в одну картку `WordGroupRow`: «run — бігти» і «run — пробіжка» стоять поруч. Заголовок — пара представника (найновішого запису), другий рядок — «Близькі за значенням» (або «Інші переклади» для легасі-бакета без атрибуції). | [ЗАРАЗ→змінено фазами 2–4] D16 | [01](01-screens.md) §6, [14](14-word-details-and-audio.md) §3.1 |
| Чесні лічильники слів | Хедер деталей — «N слів · M перекладів» (друга частина зникає, коли карток стільки ж, скільки рядків); плитка на головній — «N перекладів» з плюралізацією, бо рахує рядки. | [ЗАРАЗ→змінено фазами 2–4] D16 | [01](01-screens.md) §5, §6 |
...
| Дублікати перекладів дозволені | Один source може мати кілька різних перекладів. Дедуп **двошаровий**: сенс-гейт у сторі (перетин `senseKeys`) + пара `source+translation` у Room. | [ЗАРАЗ→змінено фазами 2–4] D16 | [03](03-data-caching.md) §6.5 |
...
| Колода флеш-карток — картка на СЕНС | До 10 pager-карток, де одиниця — **сенс-група**, а не рядок: три збережені переклади одного значення дають ОДНУ картку. Лице — пара представника, зворот — його переклад + рядок «також: …» з рештою збережених перекладів сенсу. | [ЗАРАЗ→змінено фазами 2–4] D16 | [01](01-screens.md) §8, [11](11-practice-training.md) §1 |
| Відбір по найслабшому члену групи | Знання сенсу = **мінімум** по його записах: сенс не вивчений, доки бодай один переклад «не знаю». Розходиться з «X% знаю» на плитці (середнє по рядках) — за дизайном. | [ЗАРАЗ→змінено фазами 2–4] D16 | [11](11-practice-training.md) §1, [01](01-screens.md) §6 |
| Дельта знань — усім членам сенсу | Бінарна відповідь однією батч-подією `AdjustSenseGroupKnowledge` рухає кожен збережений переклад значення; кламп 0..100 по кожному окремо; 1 sync і 1 подія аналітики на картку. | [ЗАРАЗ→змінено фазами 2–4] D16 | [11](11-practice-training.md) §1, [18](18-analytics.md) |
| Легасі-бакет тренується разом | Записи без атрибуції дають одну «легасі»-групу на слово, хоч це можуть бути різні значення — «Знаю» підіймає їх усі. Відома межа; лікується бекфілом атрибуції (наразі відсутнім). | [ЗАРАЗ] | [11](11-practice-training.md) §1, §12 |
...
| Поточний відбір (найслабші) | shuffle(seed) → sort by knowledge% → take(10) **сенсів**. Причина «ті самі 10». | [ЗАРАЗ] | [11](11-practice-training.md) §1 |
...
| Сесія 10 сенсів (≈7 повтор + ≈3 нові) | `SESSION_SIZE=10`, `NEW_PER_SESSION=3`; адаптивні квоти. Одиниця квоти — сенс-група (D16), не рядок: 12 рядків можуть дати 6 карток. | [НОВЕ] D10 + D16 | [11](11-practice-training.md) §4.2 |
...
### 4.1 [НОВЕ] — затверджені зміни (D1–D16)
...
| # | Док | Домени |
|---|---|---|
| 00 | [Огляд і рішення](00-overview-and-decisions.md) | Призначення, стани, глосарій, константи, D1–D16 |
